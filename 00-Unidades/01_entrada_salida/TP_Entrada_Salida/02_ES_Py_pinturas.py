...
class App(customtkinter.CTk):
    ...
    def btn_convertir_c_f_on_click(self):
        temp_c_str = self.txt_temperatura_c.get()
        temp_c = float(temp_c_str)
        temp_f = (temp_c * (9/5) + 32)
        mensaje = "La temperatura en Fahrenheit es: {0}".format(temp_f)
        alert("Temperatura", mensaje)

    def btn_convertir_f_c_on_click(self):
        temp_f_str = self.txt_temperatura_f.get()
        temp_f = float(temp_f_str)
        temp_c = ((temp_f - 32) * 5/9)
        mensaje = "La temperatura en Centígrados es: {0}".format(temp_c)
        alert("Temperatura" , mensaje)
